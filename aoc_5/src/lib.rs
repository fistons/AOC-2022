    ...
    for line in stacks.lines().rev().skip(1) {
        ...
        line.chars().collect::<Vec<char>>()
            ...
            .for_each(|x| {
                ...
                if let Some(c) = m.pop() {
                    map.entry(index).or_default().push(c.chars().next().unwrap());
                }
                index += 1;
            });
    }
...
impl fmt::Debug for Move {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "Move {} elements from {} to {}", self.number, self.from, self.to)
    }
}
...
fn moves_crates_9000(stacks: &mut HashMap<usize, Vec<char>>, m: &Move) {
    for _ in 0..m.number { //TODO: use drain() instead?
...
pub fn part1(input_path: &str) -> anyhow::Result<String> {
    ...
    let (initial_stacks, moves) = input
        .split_once("\n\n").context("Could not split input")?;

    let mut stacks = parse_initials_stacks(initial_stacks)?;
    read_moves(moves)?.iter().for_each(|m| moves_crates_9000(&mut stacks, m));


    let result = stacks.keys().sorted().map(|key| stacks[key].last().unwrap()).collect::<String>();
...
pub fn part2(input_path: &str) -> anyhow::Result<String> {
    ...
    let mut stacks = parse_initials_stacks(initial_stacks)?;
    read_moves(moves)?.iter().for_each(|m| moves_crates_9001(&mut stacks, m));

    Ok(stacks.keys().sorted().map(|key| stacks[key].last().unwrap()).collect::<String>())
}
